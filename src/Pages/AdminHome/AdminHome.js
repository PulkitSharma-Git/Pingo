...
const AllFlightsPage = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get('backend api /api/flight/getAllflights', {
        headers: {
          Authorization: token,
  ...
  const response  = await axios.get(`backend api /api/flight/deleteflight/${flightId}`, {
        headers: {
          Authorization: token,
        },
      })
  ...
  const handleFormSubmit = async (formData) => {
    try {
      ...
      const response = await axios.post('backend api /api/flight/createflight', formData, {
        headers: {
          Authorization: token,
          'Content-Type': 'application/json',
        },
